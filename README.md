...
Rasta Pyramid Factory is a Windows-first raster manufacturing tool built around **QGIS 3.44.9 as a headless rendering engine**. Give it a very large flat image or a genuinely georeferenced raster and it manufactures a verified multiscale raster pyramid.
...
The current live-proven Rasta core remains **v0.1.3**:
...
- Atlantic synthetic parking for ordinary flat images;
...
A later `Rasta_Pyramid_Factory_v0_1_4_TEST` branch changed the finished-product selector to three independent choices:
...
The REST branch came from the Map Fountain / Static REST WMTS exploration. Its selection logic and converter path are **BUILT / SELF-TESTED**, not Rasta's live-proven baseline.

Do not describe v0.1.4 REST output as LIVE-PROVEN until the real intended targets pass it.
...
| v0.1.3 automatic synthetic placement + heartbeat | ✅ **LIVE-PROVEN** |
| v0.1.4 independent TPKX / MBTiles / REST selection | 🟡 **BUILT / SELF-TESTED** |
| v0.1.4 REST output on real mobile target | 🟡 **NOT YET LIVE-PROVEN** |

---

## Operator workflow

For the live-proven core idea:

```text
1. Choose giant raster
2. Choose zoom range
3. Choose the finished product(s)
...
verified raster MBTiles
        ↓
        ├── preserve/publish MBTiles when selected
        └── optional frozen Compact Cache V2 converter → TPKX

v0.1.4 TEST additionally branches verified MBTiles → experimental REST output
```
...
This matters because large zoomable raster viewers are hungry for data. A local pyramid gives the viewer the needed resolution levels without making every navigation move wait on a network request.
...
## Deployment relationship changed after the router experiments

Rasta manufactures pixels. It does **not** own field-delivery hardware.

The project family now separates responsibilities:

### [Offline GeoStack](https://github.com/Jim-dc95811/Offline-GeoStack)

Master field-mapping / TPKX manufacturing architecture.

### [Map Fountain](https://github.com/Jim-dc95811/Map-Fountain)

Live-proven router/storage delivery experiments. Now **PROVEN / PARKED** from the primary personal-phone path, with a possible future Starlink/basecamp NAS role.

### [Android Field Maps + ArcGIS Earth](https://github.com/Jim-dc95811/Android-Field-Maps-and-ArcGIS-Earth-)

Current personal-phone / microSD deployment work.

This creates an interesting downstream use for Rasta: spare SD-card capacity can carry deep-zoom cityscapes, historical maps, specialty scans, or other useful single-raster pyramids—but Rasta itself remains a manufacturing project.
